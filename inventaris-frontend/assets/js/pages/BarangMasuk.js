...
export default {
    template: `
        <div class="p-8">
            <header class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800">📥 Transaksi Barang Masuk</h1>
                    <p class="text-gray-500 mt-1">Catat dan pantau pasokan barang yang masuk ke gudang.</p>
                </div>
                <button @click="bukaForm" class="bg-emerald-600 text-white px-5 py-2.5 rounded-lg shadow-md hover:bg-emerald-700 transition font-medium">
                    + Catat Barang Masuk
                </button>
            </header>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        <tr v-for="item in riwayat" :key="item.id" class="border-b border-gray-50 hover:bg-gray-50/50">
                            <td class="p-4 text-gray-600">{{ item.tanggal_masuk }}</td>
                            <td class="p-4 font-semibold text-primary">{{ item.kode_barang }}</td>
                            <td class="p-4 font-medium">{{ item.nama_barang }}</td>
                            <td class="p-4 text-center font-bold text-emerald-600">+{{ item.jumlah }}</td>
                            <td class="p-4 text-gray-500 text-sm">{{ item.keterangan || '-' }}</td>
                        </tr>
                        <tr v-if="riwayat.length === 0">
                            <td colspan="5" class="p-8 text-center text-gray-400">Belum ada riwayat barang masuk.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div v-if="showModal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                    <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                        <h3 class="text-xl font-bold text-gray-800">Form Barang Masuk</h3>
                    ...
                    <form @submit.prevent="simpanTransaksi" class="p-6">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.tanggal_masuk" type="date" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" required>
                            </div>
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Keterangan / Catatan</label>
                            <textarea v-model="form.keterangan" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" rows="2" placeholder="Contoh: Restock gudang bulanan..."></textarea>
                        </div>
                        
                        <div class="flex justify-end gap-3">
                            <button type="button" @click="tutupForm" class="px-5 py-2 rounded-lg bg-gray-100 font-medium">Batal</button>
                            <button type="submit" class="px-5 py-2 rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 font-medium">Simpan Transaksi</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    setup() {
        ...
        const form = ref({ barang_id: '', jumlah: 1, tanggal_masuk: '', keterangan: '' });

        const loadData = async () => {
            try {
                const resMasuk = await api.get('/barang-masuk'); riwayat.value = resMasuk.data;
                const resBarang = await api.get('/barang'); barangs.value = resBarang.data;
            } catch (error) { console.error(error); }
        };

        const bukaForm = () => {
            // Set default tanggal hari ini agar user tidak repot ngisi manual
            const hariIni = new Date().toISOString().split('T')[0];
            form.value = { barang_id: '', jumlah: 1, tanggal_masuk: hariIni, keterangan: '' };
        ...
        const simpanTransaksi = async () => {
            try {
                await api.post('/barang-masuk', form.value);
                tutupForm();
                loadData();
            } catch (error) {
                alert("Gagal memproses transaksi barang masuk!");
            }
        };
